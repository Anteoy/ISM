...
var tr=$("tr",t);
if(_5c.showPageList){
var ps=$("<select class=\"pagination-page-list\" style=\"display:none\"></select>");
...
$("<td><input class=\"pagination-row\" type=\"text\" value='"+_5c.pageSize+"' size=\"2\"></td>").appendTo(tr);
...
_5d.find("input.pagination-row").unbind(".pagination").bind("keydown.pagination",function(e){
	if(e.keyCode==13){
		var _61=parseInt($(this).val())||1;
		_5c.pageSize=_61;
		$(this).val(_5c.pageSize);
...
},beforePageText:"当前页",afterPageText:"/总共{pages}页",displayMsg:"当前显示{from}-{to}条 / 总共{total}条"};
})(jQuery);
(function($){
function _7d(_7e){
var _7f=$(_7e);
...
if(_d0.state=="open"){
$("<span class=\"tree-icon\"></span>").addClass(_d0.iconCls||"tree-folder tree-folder-open").prependTo(_d1);
$("<span class=\"tree-hit tree-expanded\"></span>").prependTo(_d1);
}else{
$("<span class=\"tree-icon\"></span>").addClass(_d0.iconCls||"tree-folder").prependTo(_d1);
...
$("<span class=\"tree-icon\"></span>").addClass(_d0.iconCls||"tree-file").prependTo(_d1);
...
var _hit=hit.next();
if (_hit.hasClass("tree-folder")) {
	_hit.addClass("tree-folder-open");
}
var ul=$(_ed).next();
if(ul.length){
if(_ef.animate){
...
var _hit=hit.next();
if(_hit.hasClass("tree-folder")){
	_hit.removeClass("tree-folder-open");
}
var ul=$(_f4).next();
if(_f5.animate){
...
},plugins:["linkbutton","menu","menubutton","splitbutton","progressbar","tree","combobox","combotree","numberbox","validatebox","searchbox","numberspinner","timespinner","calendar","datebox","datetimebox","slider","layout","panel","datagrid","propertygrid","treegrid","tabs","accordion","window","dialog","simplegrid"],parse:function(_168){
var aa=[];
...
//tip.css({display:"block",left:box.offset().left,top:box.offset().top+20});
...
hit.removeClass("tree-collapsed tree-collapsed-hover").addClass("tree-expanded");var _hit = hit.next();
if(_hit.hasClass("tree-folder")){
	_hit.addClass("tree-folder-open");
}
...
cc.push("<span class=\"tree-indent\"></span>");
cc.push("<span class=\"tree-icon "+(row.iconCls?row.iconCls:"tree-file")+"\"></span>");
}
}
